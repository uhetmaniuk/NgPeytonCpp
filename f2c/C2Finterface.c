...
void ldlt_fact__(int *token, int *colptr, int *rowind, double *nzvals);
...
extern void bfinit_(int *neqns, int *nsuper, int *xsuper,
         int *snode, int *xlindx, int *lindx, int *cachsz,
        int *tmpsiz, int *split);

extern void blkdinv_(int *nsuper, int *xsuper, 
        int *xlindx, int *lindx, int *xlnz, 
        double *lnz, double *dinvp, double *y, 
        int *mark, int *nzidx, int *perm);

extern void blkfct_(int *neqns, int *nsuper,
         int *nunrol, int *xsuper, int *snode, int *split,
        int *xlindx, int *lindx, int *xlnz, double *lnz,
        double *diag, int *iwsiz, int *iwork, int *tmpsiz,
        double *tmpvec, int *iflag);

extern void blkslv_(int *nsuper, int *xsuper,
        int *xlindx, int *lindx, int *xlnz, 
...
extern void inpnv_(int *neqns, int *xadjf,
        int *adjf, double *anzf, int *perm, int *invp,
        int *nsuper, int *xsuper, int *xlindx, int *lindx,
        int *xlnz, double *lnz, int *iwsiz, int *offset,
        int *iflag);
...
extern void ordmmd_(int *n,      int *xlindx, int *lindx, 
                    int *invp,   int *perm,   int *iwmax,  int *iwork,
                    int *nnzl,   int *nsub,   int *colcnt, int *nsuper,
                    int *xsuper, int *snodes, int *sfiflg, int *iflag); 

extern void sfinit_(int *neqns, int *nnza,
        int *xadj, int *adjncy, int *perm, int *invp,
        int *colcnt, int *nnzl, int *nsub, int *nsuper, 
        int *snode, int *xsuper, int *iwsiz, int *iwork,
        int *iflag);

extern void symfct_(int *n,      int *nnza,   int *xadj,
	  ...
	  mat[*token].adj[nnz-1]); */
       ...
       if (*order < 0 || *order >1) {
        ...
        // mat[*token].invp[perm[i]-1] = i+1;
           mat[*token].perm[i] = i + 1;
           mat[*token].invp[i] = i + 1;
          }
       }
       free(xadj2);
       free(adj2);
...
void ldlt_fact__(int *token, int *colptr, int *rowind, double *nzvals)
{
    int   i, nnz, nnzl, neqns, nsuper, tmpsiz, iflag, iwsiz;
