...
int main(int argc, char **argv) {
    ...
    mesh.resize(nnodes);
    for (i = 0; i < nnodes; i++) {
        mesh[i] = i + 1;  // 1-based node numbering
    }
    ...
    colptr[0] = 1;  // 1-based column pointers
    node = 0;
    ...
    for (j = 0; j < ny; j++) {
        for (i = 0; i < nx; i++) {
            ...
            colptr[node + 1] = int(rowind.size()) + 1;  // 1-based
            node++;
        }
    }
    ...
    NgPeytonCpp::SymmetricSparse<Scalar> LDLt(nnodes, colptr.data(),
                                              rowind.data(), -1);
    LDLt.ldlTFactorize(colptr.data(),
    ...
    // SpMV: Ax = A*x (adjust for 1-based colptr/rowind)
    for (i = 0; i < nnodes; ++i) {
        Ax[i] = 0;
        for (auto k = colptr[i] - 1; k < colptr[i + 1] - 1; ++k) {
            Ax[i] += nzvals[k] * x[rowind[k] - 1];
        }
    }
