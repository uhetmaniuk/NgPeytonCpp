#ifndef NGPEYTONCPP_DETAILS_ORDERING_H
#define NGPEYTONCPP_DETAILS_ORDERING_H
...
int mmdupd(
  Index ehead, Index neqns, const Index* xadj, Index* adjncy, Index delta,
...
template <typename Index = int64_t>
void mmdint(
  Index neqns, const Index* xadj, Index* dhead, Index* dforw, Index* dbakw,
  Index* qsize, Index* llist, Index* marker) {
  /* Local variables */
...
template <typename Index = int64_t>
int mmdelm(
  Index mdnode, const Index* xadj, Index* adjncy, Index* dhead, Index* dforw,
  Index* dbakw, Index* qsize, Index* llist, Index* marker, Index maxint,
  Index tag) {
  /* Local variables */
...
void genmmd(
  Index neqns, const Index* xadj, Index* adjncy, Index* invp, Index* perm,
  Index delta, Index* dhead, Index* qsize, Index* llist, Index* marker,
  Index maxint, Index& nnzl, Index& nofsub, Index* colcnt, Index& nsuper,
  Index* xsuper, Index* snode) {
  /* Local variables */
...
L800:
  mmdelm(
    mdnode, &xadj[1], &adjncy[1], &dhead[1], &invp[1], &perm[1], &qsize[1],
    &llist[1], &marker[1], maxint, tag);
  num += qsize[mdnode];
  llist[mdnode] = ehead;
  ehead = mdnode;
  if (delta >= 0) {
...
L900:
  ...
  mmdupd(
    ehead, neqns, &xadj[1], &adjncy[1], delta, mdeg, &dhead[1], &invp[1],
...
void ordmmd(
  Index neqns, const Index* xadj, Index* adjncy, Index* invp, Index* perm,
  Index iwsiz, Index* iwork, Index& nnzl, Index& nofsub, Index* colcnt,
  Index& nsuper, Index* xsuper, Index* snode, bool& sfiflg, Index& iflag) {
  /* Local variables */
  Index delta, maxint;
  ...
  maxint = 32767;
  genmmd(
    neqns, xadj, adjncy, invp, perm, delta, iwork, &iwork[neqns],
    &iwork[2 * neqns], &iwork[3 * neqns], maxint, nnzl, nofsub, colcnt, nsuper,
    xsuper, snode);
  sfiflg = false;
} /* ordmmd */
